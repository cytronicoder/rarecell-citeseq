...
# Also make scripts/ importable so run_pipeline can be reused without duplication
...
try:
    from run_benchmark import run_pipeline  # noqa: E402
except ImportError as _import_err:
    raise ImportError(
        "Could not import run_pipeline from scripts/run_benchmark.py. "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Auto-detected from common column names if not found."
        ),
    )
    parser.add_argument(
        "--target-label",
        ...
        help=(
            ...
            "If omitted, auto-selected as the second-smallest class."
        ),
    )
    parser.add_argument(
        "--representations",
...
def _infer_smoke_prefix(input_path: Path, target_label: str, explicit_prefix: str | None) -> str:
    if explicit_prefix:
        return explicit_prefix
    display = _display_path(input_path).lower()
    stem = "smoke_test_pbmc5k" if "pbmc5k" in display else "smoke_test"
    slug = re.sub(r"[^A-Za-z0-9]+", "_", str(target_label)).strip("_").lower()
...
def main() -> None:
    ...
    print(f"Label key: {label_key!r}, target label: {target_label!r}")
    ...
    output_prefix = _infer_smoke_prefix(input_path, target_label, args.output_prefix)
